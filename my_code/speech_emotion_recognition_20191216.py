...
# save to csv
def save_csv(name, data):
    with open('/home/mmnlab/Documents/audio_emotion_result/speech_emotion_recognition_RNN_2/' + str(name) + '.csv', 'wb') as csvfile:
        writer = csv.writer(csvfile)
        for rows in data:
...
if __name__ == '__main__':
    ...
    mypath = "/home/mmnlab/Documents/audio/EMODB_validation/Actor_07_stereo/" + sys.argv[1]  # original
    ...
    for i in range(100):
        ...
        most_result.append(np.argmax(np.bincount(p_result)))    # 統計鎮列裡最多的數值，存入陣列
        print (most_result)
    ...
    print (np.argmax(np.bincount(most_result)))
    print (all_np(most_result))
